...
const intParser = (from: string) => {
  const r = parseInt(from);
  return isNaN(r) ? null : r;
};
...
const parsers: {
  [P in keyof RouteParams]: (fromParams: string) => RouteParams[P] | null;
} = {
  lat: floatParser,
  lon: floatParser,
  zoom: floatParser,
...
const toTruncated =
  (dps = 6) =>
  (value: number | null) =>
    value !== null ? round(value, dps).toString() : null;
const serializers: {
  [P in keyof RouteParams]: (from: RouteParams[P]) => string | null;
} = {
  lat: toTruncated(),
  lon: toTruncated(),
  ...
  overlays: (r) => r.join(","),
  page: (p) => p,
  editingFeature: (f) => (f === null ? null : f.toString()),
  lla: toTruncated(),
  llo: toTruncated(),
  lab: (p) => p,
};
