...
import { TOPO50 } from "./topo50MapSheets";
...
// NZMG (NZMS 260): single letter + 1-2 digits, e.g., J34
export const referenceRegex = /^\s*([a-zA-Z]\d{1,2})\s?(\d{5}[^0-9]?\d{5}|\d{4}\s?\d{4}|\d{3}[^0-9]?\d{3})\s*$/

// Topo50 (NZTM): two letters + 1-2 digits, e.g., BW18
export const topo50ReferenceRegex = /^\s*([a-zA-Z]{2}\d{1,2})\s?(\d{5}[^0-9]?\d{5}|\d{4}\s?\d{4}|\d{3}[^0-9]?\d{3})\s*$/
...
export const convertTopo50ReferenceToLatLng = (reference: string): [number, number] | undefined => {
    const match = topo50ReferenceRegex.exec(reference);
    if (!match) return;

    const sheet = match[1].toUpperCase()

    const matchingMap = TOPO50.find(n => n.sheet === sheet)
    if (!matchingMap) return;
    ...
    const [lng, lat] = proj4('EPSG:2193', 'WGS84', result)
    return [lat, lng]
}
